...
def main(win_title='test'):
    ...
    model = tensorflow.keras.models.load_model('keras_models/mymodel.model')
    label = get_label('keras_models/labels.txt')
    data = np.ndarray(shape=(1, 224, 224, 3), dtype=np.float32)

    print('Start Stream ...')
    fps = -1
    stream = 'nvarguscamerasrc ! video/x-raw(memory:NVMM), width=%d, height=%d, format=(string)NV12, framerate=(fraction)%d/1 !nvvidconv flip-method=%d ! nvvidconv ! video/x-raw, width=(int)%d, height=(int)%d, format=(string)BGRx ! videoconvert ! appsink' % (1920, 1080, 60,0, 640, 480)
